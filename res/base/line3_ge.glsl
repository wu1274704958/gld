...
void main() {
    ...
    vec4 p2 = vec4(_2,gl_in[1].gl_Position.w);
     ...
     for(int i = 0;i < seg;++i)
     {
        ...
        emit(p1 + vec4(first, 0.f),vec2(0.0f,0.5f - cos(-curr_ang) * 0.5f ),vs_out[0].f_color);
        emit(p1 ,vec2(hc_rate,0.5f),vs_out[0].f_color);
     ...
     for(int i = 0;i < seg;++i)
     {
        ...
        emit(p2 ,vec2(1.0f - hc_rate,0.5f),vs_out[1].f_color);
        emit(p2 + vec4(first, 0.f),vec2(0.0f, 0.5f - cos(-curr_ang) * 0.5f ),vs_out[1].f_color);
