...
public:
    Demo1() : view_pos("view_pos"), perspective("perspective"), world("world") ,time_("time"),refract_rate("refract_rate")
        {}
    int init() override
    {
        RenderDemoRotate::init();
        
        // program = DefDataMgr::instance()->load<DataType::Program>("geometry/base_fg.glsl","geometry/explode.glsl");
        program = DefDataMgr::instance()->load<DataType::ProgramWithGeometry>("geometry/base_vs.glsl","reflect/refract_fg.glsl","geometry/explode.glsl");
        program->use();

        program->locat_uniforms("perspective", "world", "model", 
            //"diffuseTex", "ambient_strength","specular_strength","shininess","specularTex",
            "skybox",
            "view_pos",
            "time",
            "refract_rate"
        ...
        refract_rate.attach_program(program);
        ...
        auto node = DefDataMgr::instance()->load<DataType::SceneWithGeometryNoMaterial>("model/hEART_and_key.blend",LoadScene::default_args(),
            "geometry/base_vs.glsl","reflect/refract_fg.glsl","geometry/explode.glsl");
        ...
        node->get_child(3)->get_comp<Transform>()->pos = glm::vec3(-9.0f,-14.5f,0.0f);       
        node->get_child(3)->get_comp<Transform>()->setRotateX(90.f);
        node->get_child(3)->get_comp<Transform>()->setRotateZ(-35.f);
...
private:
    ...
    GlmUniform<UT::Float> time_,refract_rate;
