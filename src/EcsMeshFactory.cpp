...
namespace gld::ecs {
    ...
    MeshHandle MeshFactory::cube(float size) {
        // 24 vertices: pos(3) + uv(2) + normal(3), 6 faces.
        const float s = size * 0.5f;
        std::vector<float> v = {
            // +Z (front)
            -s,-s, s, 0,0, 0,0,1,   s,-s, s, 1,0, 0,0,1,   s, s, s, 1,1, 0,0,1,  -s, s, s, 0,1, 0,0,1,
            // -Z (back)
             s,-s,-s, 0,0, 0,0,-1,  -s,-s,-s, 1,0, 0,0,-1,  -s, s,-s, 1,1, 0,0,-1,   s, s,-s, 0,1, 0,0,-1,
            // +X (right)
             s,-s, s, 0,0, 1,0,0,   s,-s,-s, 1,0, 1,0,0,   s, s,-s, 1,1, 1,0,0,   s, s, s, 0,1, 1,0,0,
            // -X (left)
            -s,-s,-s, 0,0, -1,0,0,  -s,-s, s, 1,0, -1,0,0,  -s, s, s, 1,1, -1,0,0,  -s, s,-s, 0,1, -1,0,0,
            // +Y (top)
            -s, s, s, 0,0, 0,1,0,   s, s, s, 1,0, 0,1,0,   s, s,-s, 1,1, 0,1,0,  -s, s,-s, 0,1, 0,1,0,
            // -Y (bottom)
            -s,-s,-s, 0,0, 0,-1,0,   s,-s,-s, 1,0, 0,-1,0,   s,-s, s, 1,1, 0,-1,0,  -s,-s, s, 0,1, 0,-1,0,
        };
        std::vector<int> idx;
        ...
        vao->buffs().get<ArrayBufferType::VERTEX>().vertex_attrib_pointer<
            VAP_DATA<3, float, false>, VAP_DATA<2, float, false>, VAP_DATA<3, float, false>>();
    ...
    MeshHandle MeshFactory::quad(float size) {
        const float s = size * 0.5f;
        std::vector<float> v = {
            -s,-s, 0, 0,0, 0,0,1,   s,-s, 0, 1,0, 0,0,1,   s, s, 0, 1,1, 0,0,1,  -s, s, 0, 0,1, 0,0,1,
        };
